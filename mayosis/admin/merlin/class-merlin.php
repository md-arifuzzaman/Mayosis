...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		// Get TGMPA.
		if ( class_exists( 'Mayosis_plugin_activation' ) ) {
			$this->tgmpa = isset( $GLOBALS['tgmpa'] ) ? $GLOBALS['tgmpa'] : Mayosis_plugin_activation::get_instance();
		}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'mayosis' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'mayosis' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'mayosis' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'mayosis' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'mayosis' ),
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'mayosis' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'mayosis' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'mayosis' ); ?>">
										<?php esc_html_e( 'req', 'mayosis' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'mayosis' ) );
	}
 ...
 */
	...
	<?php echo wp_kses( $this->svg( array( 'icon' => 'license' ) ), $this->svg_allowed_html() ); ?>
     ...
     <?php if ( class_exists( 'mayosis_core' ) ) { ?>
			...
			<footer class="merlin__content__footer <?php echo esc_attr( $class ); ?>">
					...
					<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next merlin__button--proceed merlin__button--colorchange"><?php esc_html_e( 'Next', 'mayosis'); ?></a>
 ...
 <?php }
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'mayosis' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'mayosis' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'mayosis' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'mayosis' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'mayosis' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'mayosis' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'mayosis' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'mayosis' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'mayosis' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'mayosis' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'mayosis' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'mayosis' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'mayosis' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'mayosis' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'mayosis' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'mayosis' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'mayosis' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'mayosis' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'mayosis' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'mayosis' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'mayosis' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'mayosis' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'mayosis' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'mayosis' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'mayosis' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'mayosis' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'mayosis' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'mayosis' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'mayosis' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'mayosis' ),
				array(
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'mayosis' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'mayosis' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'mayosis' ),
				'description'      => esc_html__( 'Demo content data.', 'mayosis' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				...
				'description'      => esc_html__( 'Sample widgets data.', 'mayosis' ),
				'pending'          => esc_html__( 'Pending', 'mayosis' ),
				'installing'       => esc_html__( 'Installing', 'mayosis' ),
				'success'          => esc_html__( 'Success', 'mayosis' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'mayosis' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'mayosis' ),
				'pending'          => esc_html__( 'Pending', 'mayosis' ),
				'installing'       => esc_html__( 'Installing', 'mayosis' ),
				'success'          => esc_html__( 'Success', 'mayosis' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'mayosis' ),
				'description'      => esc_html__( 'Sample theme options data.', 'mayosis' ),
				'pending'          => esc_html__( 'Pending', 'mayosis' ),
				'installing'       => esc_html__( 'Installing', 'mayosis' ),
				'success'          => esc_html__( 'Success', 'mayosis' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'mayosis' ),
				'description'      => esc_html__( 'Redux framework options.', 'mayosis' ),
				'pending'          => esc_html__( 'Pending', 'mayosis' ),
				'installing'       => esc_html__( 'Installing', 'mayosis' ),
				'success'          => esc_html__( 'Success', 'mayosis' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'mayosis' ),
				'description'      => esc_html__( 'After import setup.', 'mayosis' ),
				'pending'          => esc_html__( 'Pending', 'mayosis' ),
				'installing'       => esc_html__( 'Installing', 'mayosis' ),
				'success'          => esc_html__( 'Success', 'mayosis' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'mayosis' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'mayosis' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( __( 'The home page was set', 'mayosis' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'mayosis' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'mayosis' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'mayosis' ), $import_file );
			}
		}
