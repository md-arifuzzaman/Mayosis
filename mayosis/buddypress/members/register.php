...
<div id="buddypress">
    ...
    <div id="item-body-register" class="mayosis-bp-group">
	    ...
	    <div id="header-cover-image-custom">
	        <h1><?php esc_html_e('Create an Account','mayosis');?></h1>
	    </div>
	...
	<div class="page container" id="register-page">
		...
		<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
			...
			do_action( 'bp_before_registration_disabled' ); ?>
				...
				<p><?php _e( 'User registration is currently not allowed.', 'mayosis' ); ?></p>
		...
		<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
			...
			<p><?php _e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'mayosis' ); ?></p>
			...
			<div class="register-section" id="basic-details-section">
				...
				<h2><?php _e( 'Account Details', 'mayosis' ); ?></h2>

				<label for="signup_username"><?php _e( 'Username', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
				<?php
				...
				<label for="signup_email"><?php _e( 'Email Address', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
				<?php
				...
				<label for="signup_password"><?php _e( 'Choose a Password', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
				<?php
				...
				<label for="signup_password_confirm"><?php _e( 'Confirm Password', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
				<?php
			...
			<?php if ( bp_is_active( 'xprofile' ) ) : ?>
				...
				<div class="register-section" id="profile-details-section">
					...
					<h2><?php _e( 'Profile Details', 'mayosis' ); ?></h2>
							...
							if ( bp_current_user_can( 'bp_xprofile_change_field_visibility' ) ) : ?>
								<p class="field-visibility-settings-toggle" id="field-visibility-settings-toggle-<?php bp_the_profile_field_id() ?>"><span id="<?php bp_the_profile_field_input_name(); ?>-2">
									...
									printf(
										...
										__( 'This field can be seen by: %s', 'mayosis' ),
										'<span class="current-visibility-level">' . bp_get_the_profile_field_visibility_level_label() . '</span>'
									);
									?>
									</span>
									<button type="button" class="visibility-toggle-link" aria-describedby="<?php bp_the_profile_field_input_name(); ?>-2" aria-expanded="false"><?php _ex( 'Change', 'Change profile field visibility level', 'mayosis' ); ?></button>
								</p>

								<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id() ?>">
									<fieldset>
										<legend><?php _e( 'Who can see this field?', 'mayosis' ) ?></legend>

										<?php bp_profile_visibility_radio_buttons() ?>

									</fieldset>
									<button type="button" class="field-visibility-settings-close"><?php _e( 'Close', 'mayosis' ) ?></button>
			...
			<?php if ( bp_get_blog_signup_allowed() ) : ?>
				...
				<div class="register-section" id="blog-details-section">
					...
					<h2><?php _e( 'Blog Details', 'mayosis' ); ?></h2>

					<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'mayosis' ); ?></label></p>

					<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

						<label for="signup_blog_url"><?php _e( 'Blog URL', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
						<?php
						...
						<label for="signup_blog_title"><?php _e( 'Site Title', 'mayosis' ); ?> <?php _e( '(required)', 'mayosis' ); ?></label>
						<?php
						...
						<fieldset class="register-site">
							<legend class="label"><?php _e( 'Privacy: I would like my site to appear in search engines, and in public listings around this network.', 'mayosis' ); ?></legend>
							<?php
							...
							<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'mayosis' ); ?></label>
							<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'mayosis' ); ?></label>
						</fieldset>
			...
			<div class="submit">
				<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'mayosis' ); ?>" />
			</div>
		...
		<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
			...
			<div id="template-notices" role="alert" aria-atomic="true">
				<?php if ( bp_registration_needs_activation() ) : ?>
					<p><?php _e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'mayosis' ); ?></p>
				<?php else : ?>
					<p><?php _e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'mayosis' ); ?></p>
				<?php endif; ?>
			</div>
